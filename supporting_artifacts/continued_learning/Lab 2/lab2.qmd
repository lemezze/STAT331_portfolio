...
##3
str(surveys)
# taken from: https://www.geeksforgeeks.org/display-the-internal-structure-of-an-object-in-r-programming-str-function/
```
...
-   The dimensions of the data are 30463 rows and 15 columns.
...
-   The data types are both numbers and characters.
...
#Code for question 5!
 ggplot(data = surveys,
  ...
  labs(x = "Weight (g)", 
       y = "Hindfoot Length (mm)",
       title = "Relationship between Weight (g) and Hindfoot Length (mm) of Rodents")
```
...
```{r jitter}
# Code for question 6!
# geom_jitter() is great for plots that have hella points. It spreads them out.
 ggplot(data = surveys,
       mapping = aes(x = weight,
                     y = hindfoot_length)) +
...
# Code for question 7
 ggplot(data = surveys,
  ...
  labs(x = "Weight (g)",
       y = "Hindfoot Length (mm)",
       title = "Relationship between Weight (g) and Hindfoot Length (mm) of Rodent Species")
```
